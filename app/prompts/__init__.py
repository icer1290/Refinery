...
1. **识别新闻时效性**：
   - 搜索结果中的 "X hours ago"、"X days ago"、"1 day ago" 等时间戳表示这是**已发布的新闻**，不是预测。
   - 如果证据显示事件已经发生（如NASA官方新闻稿确认），则应认定该事件为事实，而非"未来预测"。
   - 不要因为日期（如2026年）看起来"在未来"就判定为幻觉——应以证据中的时间戳为准。

2. **全面检查所有来源**：事实依据可以来自【文章上下文】（包括标题、摘要）或【已收集的信息】（搜索结果）。两者都是有效证据来源。

3. **区分事实与推测**：只标记真正没有证据支持的编造内容，不要将基于证据的内容误判为幻觉。

4. **数据核对标准**：
   - 数据必须与证据一致，但允许合理的舍入误差（如252,756和252,757英里的差异属于舍入误差，不是幻觉）。
   - 数字差异超过1%才需要标记为问题。

5. **正确归属引用**：区分来自文章摘要的事实和来自搜索结果的事实。
...
**注意**：文章摘要中的信息也是有效的事实来源，回复中使用文章摘要中的内容是有依据的。
...
1. **确定时间背景**：检查搜索结果的时间戳（如 "X days ago"），确定新闻事件是否已发生。
2. **识别回复中的所有事实陈述**：列出回复中提到的具体事实、数据、名称、事件等。
3. **逐一验证每个事实**：
   - 首先检查是否在【文章摘要】中有依据
   - 然后检查是否在【已收集的信息】中有依据
   - 只有当两者都没有依据时，才判定为"幻觉"
4. **验证引用准确性**：检查引用的来源是否确实包含该信息
