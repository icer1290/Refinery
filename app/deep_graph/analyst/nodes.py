...
1. fetch_articles_node: Fetch article details for analysis
2. fetch_seed_subgraph_node: Get entities/relationships from selected articles
3. expand_subgraph_node: Find 1-hop neighbors with relevance scoring
4. build_visualization_node: Create graph nodes/edges for visualization
5. generate_report_node: LLM generates comprehensive analysis report
"""
...
async def fetch_articles_node(
    ...
    runtime: Runtime[DeepGraphAnalystContext],
) -> dict[str, Any]:
    """Fetch article details for analysis.

    Args:
        state: Current analyst state
        runtime: LangGraph runtime with DeepGraphAnalystContext
...
async def expand_subgraph_node(
    ...
    """
    session = runtime.context.session
    max_hops = runtime.context.max_hops
    expansion_limit = runtime.context.expansion_limit
    ...
    try:
        ...
        all_expanded_rels = []
        # Track all expanded entity objects for visualization
        all_expanded_entity_objects: list[dict[str, Any]] = []
        ...
        for hop in range(max_hops):
            ...
            remaining_limit = expansion_limit - len(expanded_entities)
            for item in scored_neighbors[:remaining_limit]:
                ...
                # Add to all expanded entity objects for visualization
                all_expanded_entity_objects.append({
                    "id": neighbor_id,
                    "name": neighbor.name,
                    ...
                    "article_ids": [str(aid) for aid in (neighbor.article_ids or [])],
                })
        ...
        return {
            ...
            "_expanded_entity_objects": all_expanded_entity_objects,
...
async def generate_report_node(
    ...
    """Generate comprehensive analysis report using LLM service.
    ...
    try:
        ...
        # Generate report
        report = await llm_service.generate_deep_graph_report(
            prompt=prompt,
            temperature=0.6,
